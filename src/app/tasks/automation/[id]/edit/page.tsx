...
export default function EditAutomationPage() {
  ...
  const { grupos, loading: gruposLoading, fetchGrupos } = useGruposHierarquicos();
  ...
  const [newRule, setNewRule] = useState({ fromStep: '', toStep: '', condition: '', action: '' });
  ...
  const loadAutomation = async () => {
    try {
      const response = await fetch(`/api/tasks/automation/rules/${params.id}`);
      if (!response.ok) {
      ...
      setForm({
        ...
        grupoHierarquicoId: automation.grupoHierarquicoId || '',
      ...
      toast({
        title: 'Erro',
        description: 'Erro ao carregar automação. Redirecionando...',
        variant: 'destructive'
      });
      router.push('/tasks');
    } finally {
  ...
  const addRule = () => {
    ...
    setNewRule({ fromStep: '', toStep: '', condition: '', action: '' });
  ...
  const validateForm = (): boolean => {
    ...
    if (!form.grupoHierarquicoId) newErrors.grupo = 'Grupo hierárquico é obrigatório';
    ...
    try {
      const response = await fetch(`/api/tasks/automation/rules/${params.id}`, {
        ...
        body: JSON.stringify({
          ...
          grupoHierarquicoId: form.grupoHierarquicoId,
          steps: form.steps,
          rules: form.rules,
      ...
      toast({
        title: 'Erro',
        description: 'Erro ao atualizar automação. Tente novamente.',
        variant: 'destructive'
      });
    } finally {
      ...
      <div className="container mx-auto py-6">
        <div className="flex items-center justify-center h-64">
          <div className="flex items-center space-x-2">
            ...
            <span>Carregando automação...</span>
          </div>
        </div>
      </div>
    );
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Editar Automação de Negócios</h1>
            <p className="text-muted-foreground">Modifique a configuração da automação</p>
          </div>
        </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="grupo">Grupo Hierárquico *</Label>
                  <Select
